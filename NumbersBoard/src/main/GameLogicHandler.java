...
public class GameLogicHandler {
	...
	private int winnerID, winnerResult;
	...
	/**
	 * Calculates the score when the game is over
	 */
			...
			for (int j = 0; j < board.size(); j++) {
				...
				if (board.get(i).get(j).getCards().size() > 0) {
					...
					if (scoresCopy[1] < scoresCopy[2]) {
						// Add one win to player which has the highest square
						// score
						...
						// If there are multiple winners assign victory to the
						// first player who place the card
						// totalPlayerScores[playersList.indexOf(cards.get(0).getOwner())]
						// += 1;
